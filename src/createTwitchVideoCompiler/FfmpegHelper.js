  ...
  async transcodeVideo(
    ...
    const ffmpeg = this.spawn(
      ...
      [
        ...
        '-preset', 'ultrafast',
        '-vf', `scale=1920:1080,
                drawtext=fontfile=${fontPath}: text=${broadcaster}: fontcolor=white:shadowx=3: shadowy=1: shadowcolor=DeepPink: fontsize=60: x=(w*.8) - (text_w / 2): y=(h*.7) - (text_h / 2),
                drawtext=fontfile=${fontPath}: text=${game}: fontcolor=white:shadowx=3: shadowy=1: shadowcolor=MediumSeaGreen: fontsize=60: x=(w*.8) - (text_w / 2): y=(h*.8) - (text_h / 2)`,
        outputPath,
        '-y',
      ],
    );
